    ...
    public class CreateConnectionStringTests
    {
        ...
        public void ShouldReturnFallbackConnectionStringWhenNotUsingEnvironmentConnectionString()
        {
            _config.Setup(s => s.GetSection("environment_connection_string"))
                .Returns(() =>
                {
                    ...
                    section.Setup(s => s.Value).Returns(null as string);
            ...
            connectionString.Should().Be("fallback connection string");
        }
        ...
        public void ShouldUseTheConfigurationToBuildConnectionString()
        {
            ...
            _config.Setup(s => s.GetSection("db_source"))
                ...
                {
                    var section = new Mock<IConfigurationSection>();
                    section.Setup(s => s.Value).Returns("db_source");
                    return section.Object;
                });
            ...
            _config.Setup(s => s.GetSection("db_catalog"))
                ...
                {
                    var section = new Mock<IConfigurationSection>();
                    section.Setup(s => s.Value).Returns("db_catalog");
                    return section.Object;
                });
            ...
            _config.Setup(s => s.GetSection("db_username"))
                ...
                {
                    var section = new Mock<IConfigurationSection>();
                    section.Setup(s => s.Value).Returns("db_username");
                    return section.Object;
                });
            ...
            _config.Setup(s => s.GetSection("db_password"))
                ...
                {
                    var section = new Mock<IConfigurationSection>();
                    section.Setup(s => s.Value).Returns("db_password");
                    return section.Object;
                });

            var connectionString = _config.Object.CreateConnectionString("fallback");

            connectionString.Should().Be("Data Source=db_source;Initial Catalog=db_catalog;Persist Security Info=False;User ID=db_username;Password=db_password;MultipleActiveResultSets=False;Connect Timeout=30;Encrypt=True;TrustServerCertificate=False;Authentication=ActiveDirectoryPassword");
        }
    }
}

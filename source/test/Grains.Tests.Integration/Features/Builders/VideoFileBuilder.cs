	...
	public class VideoFileBuilder
	{
		...
		public void GivenIHaveValidVideos(int numberOfFilms, string invalid)
		{
			...
			Func<int, string> buildName = identifier =>
			                              {
				                              var seasonNumber = random.Next(1, 99)
				                                                       .ToString("D2");
				                              var episodeNumber = random.Next(1, 99)
				                                                        .ToString("D2");
				                              ...
				                              var name =
					                              $"Title BluRay {invalid} {identifier.ToString("D2")} (2019){(invalid == "invalid" ? invalidAddition : string.Empty)}.mkv";
				                              return name;
			                              };
